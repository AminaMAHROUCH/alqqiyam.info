...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.uniteRegional.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.unite-regionals.update", [$uniteRegional->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name_complet">{{ trans('cruds.uniteRegional.fields.name_complet') }}</label>
                <input class="form-control {{ $errors->has('name_complet') ? 'is-invalid' : '' }}" type="text" name="name_complet" id="name_complet" value="{{ old('name_complet', $uniteRegional->name_complet) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tel_1') ? 'is-invalid' : '' }}" type="text" name="tel_1" id="tel_1" value="{{ old('tel_1', $uniteRegional->tel_1) }}" required>
                @if($errors->has('tel_1'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tel_2') ? 'is-invalid' : '' }}" type="text" name="tel_2" id="tel_2" value="{{ old('tel_2', $uniteRegional->tel_2) }}">
                @if($errors->has('tel_2'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email_personnel') ? 'is-invalid' : '' }}" type="email" name="email_personnel" id="email_personnel" value="{{ old('email_personnel', $uniteRegional->email_personnel) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('region') ? 'is-invalid' : '' }}" name="region_id" id="region_id">
                    @foreach($regions as $id => $region)
                        <option value="{{ $id }}" {{ (old('region_id') ? old('region_id') : $uniteRegional->region->id ?? '') == $id ? 'selected' : '' }}>{{ $region }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('province') ? 'is-invalid' : '' }}" name="province_id" id="province_id">
                    @foreach($provinces as $id => $province)
                        <option value="{{ $id }}" {{ (old('province_id') ? old('province_id') : $uniteRegional->province->id ?? '') == $id ? 'selected' : '' }}>{{ $province }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('profession') ? 'is-invalid' : '' }}" type="text" name="profession" id="profession" value="{{ old('profession', $uniteRegional->profession) }}" required>
