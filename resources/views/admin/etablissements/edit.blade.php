...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.etablissement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.etablissements.update", [$etablissement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name_complet">{{ trans('cruds.etablissement.fields.name_complet') }}</label>
                <input class="form-control {{ $errors->has('name_complet') ? 'is-invalid' : '' }}" type="text" name="name_complet" id="name_complet" value="{{ old('name_complet', $etablissement->name_complet) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tel_1') ? 'is-invalid' : '' }}" type="text" name="tel_1" id="tel_1" value="{{ old('tel_1', $etablissement->tel_1) }}" required>
                @if($errors->has('tel_1'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tel_2') ? 'is-invalid' : '' }}" type="text" name="tel_2" id="tel_2" value="{{ old('tel_2', $etablissement->tel_2) }}">
                @if($errors->has('tel_2'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fix') ? 'is-invalid' : '' }}" type="text" name="fix" id="fix" value="{{ old('fix', $etablissement->fix) }}">
                @if($errors->has('fix'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email_professionel') ? 'is-invalid' : '' }}" type="email" name="email_professionel" id="email_professionel" value="{{ old('email_professionel', $etablissement->email_professionel) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email_personnel') ? 'is-invalid' : '' }}" type="email" name="email_personnel" id="email_personnel" value="{{ old('email_personnel', $etablissement->email_personnel) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('direction') ? 'is-invalid' : '' }}" name="direction_id" id="direction_id">
                    @foreach($directions as $id => $direction)
                        <option value="{{ $id }}" {{ (old('direction_id') ? old('direction_id') : $etablissement->direction->id ?? '') == $id ? 'selected' : '' }}>{{ $direction }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unite') ? 'is-invalid' : '' }}" name="unite_id" id="unite_id">
                    @foreach($unites as $id => $unite)
                        <option value="{{ $id }}" {{ (old('unite_id') ? old('unite_id') : $etablissement->unite->id ?? '') == $id ? 'selected' : '' }}>{{ $unite }}</option>
                    @endforeach
                </select>
                @if($errors->has('unite'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profession') ? 'is-invalid' : '' }}" name="profession_id" id="profession_id">
                    @foreach($professions as $id => $profession)
                        <option value="{{ $id }}" {{ (old('profession_id') ? old('profession_id') : $etablissement->profession->id ?? '') == $id ? 'selected' : '' }}>{{ $profession }}</option>
                    @endforeach
                </select>
